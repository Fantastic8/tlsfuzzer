...
def main():
    ...
    print("This test script checks if the server correctly handles malformed")
    print("Client Key Exchange messages in RSA key exchange.")
    print("When executed with `-i` it will also verify that different errors")
    print("are rejected in the same amount of time; it checks for timing")
    print("sidechannel.")
    print("The script executes tests without \"sanity\" in name multiple")
    print("times to estimate server response time.")
    print("")
    print("Quick reminder: when encrypting a value using PKCS#1 v1.5 standard")
    print("the plaintext has the following structure, starting from most")
    print("significant byte:")
    print("- one byte, the version of the encryption, must be 0")
    print("- one byte, the type of encryption, must be 2 (is 1 in case of")
    print("  signature)")
    print("- one or more bytes of random padding, with no zero bytes. The")
    print("  count must equal the byte size of the public key modulus less")
    print("  size of encrypted value and 3 (for version, type and separator)")
    print("  For signatures the bytes must equal 0xff")
    print("- one zero byte that acts as separator between padding and")
    print("  encrypted value")
    print("- one or more bytes that are the encrypted value, for TLS it must")
    print("  be 48 bytes long and the first two bytes need to equal the")
    print("  TLS version advertised in Client Hello")
    print("")
    print("All tests should exhibit the same kind of timing behaviour, but")
    print("if some groups of tests are inconsistent, that points to likely")
    print("place where the timing leak happens:")
    print("- the control group, lack of consistency here points to Lucky 13:")
    print("  - 'invalid MAC in Finished on pos 0'")
    print("  - 'invalid MAC in Finished on pos -1'")
    print("  - 'invalid padding_length in Finished'")
    print("  - 'fuzzed pre master secret' - this will end up with random")
    print("    plaintexts in record with Finished, most resembling a randomly")
    print("    selected PMS by the server")
    print("- different PKCS#1 v1.5 padding types, inconsistent results here")
    print("  may be caued by the same code used for decryption and signature")
    print("  verification:")
    print("  - 'set PKCS#1 padding type to 3'")
    print("  - 'set PKCS#1 padding type to 1'")
    print("- incorrect size of encrypted value (pre-master secret),")
    print("  inconsistent results here suggests that the decryption leaks")
    print("  length of plaintext:")
    print("  - 'zero byte in random padding' - this creates a PMS that's 4")
    print("    bytes shorter than the public key size and has a random TLS")
    print("    version")
    print("  - 'zero byte in last byte of random padding' - this creates a")
    print("    PMS that's one byte too long (49 bytes long), with a TLS")
    print("    version that's (0, major_version)")
    print("  - 'no null separator in padding' - as the PMS is all zero, this")
    print("    effectively sends a PMS that's 45 bytes long with TLS version")
    print("    of (0, 0)")
    print("  - 'two byte long PMS (TLS version only)'")
    print("  - 'one byte encrypted value' - the encrypted value is 3, as a")
    print("    correct value for first byte of TLS version")
    print("  - 'too short (47-byte) pre master secret'")
    print("  - 'very short (4-byte) pre master secret'")
    print("  - 'too long (49-byte) pre master secret'")
    print("  - 'very long (124-byte) pre master secret'")
    print("  - 'very long (96-byte) pre master secret'")
    print("- invalid PKCS#1 v1.5 encryption padding:")
    print("  - 'zero byte in first byte of random padding' - this is a mix")
    print("    of too long PMS and invalid padding, it actually doesn't send")
    print("    padding at all, the padding length is zero")
    print("  - 'invalid version number in padding' - this sets the first byte")
    print("    of plaintext to 1")
    print("  - 'no null separator in encrypted value' - this doesn't send a")
    print("    null byte separating padding and encrypted value")
    print("  - 'no encrypted value' - this sends a null separator, but it's")
    print("    the last byte of plaintext")
    print("  - 'too short PKCS padding' - this sends the correct encryption")
    print("    type in the padding (2), but one byte later than required")
    print("  - 'very short PKCS padding (40 bytes short)' - same as above")
    print("    only 40 bytes later")
    print("  - 'too long PKCS padding' this doesn't send the PKCS#1 v1.5")
    print("    version at all, but starts with padding type")
    print("- invalid TLS version in PMS, differences here suggest a leak in")
    print("  code checking for correctness of this value:")
    print("  - 'wrong TLS version (2, 2) in pre master secret'")
    print("  - 'wrong TLS version (0, 0) in pre master secret'")
    print(20 * '=')
